...
class ReleaseTests(unittest.TestCase):
    ...
    def test_version_metadata_is_synchronized(self) -> None:
        self.assertIn("VERSION 2.4.3", (ROOT / "CMakeLists.txt").read_text())
        self.assertEqual(json.loads((ROOT / "vcpkg.json").read_text())["version-semver"], VERSION)
        version_header = (ROOT / "src/version.h").read_text()
        self.assertIn('String = "2.4.3"', version_header)
        self.assertIn('(static_cast<std::uint32_t>(Major) << 24)', version_header)
        self.assertTrue((ROOT / "README.md").read_text().startswith("# NPC Pathing NG v2.4.3"))
        self.assertTrue((ROOT / "package/README.md").read_text().startswith("# NPC Pathing NG v2.4.3"))
